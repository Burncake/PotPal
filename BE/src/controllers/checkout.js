...
const updateCartQuantity = async (req, res) => {
  ...
  try {
    ...
    await cartMethods.updateProductQuantityInCart(
      cart.cartID,
      prodID,
      quantity
    );
...
const convertCartToOrder = async (req, res) => {
  const { userID, phoneNumber, address, payMethod, note, discountID } =
    req.body;
...
const getAllOrdersByUserID = async (req, res) => {
  ...
  try {
    const orders = await db("orders").where("cusID", userID).select("*");
...
const getCartByCustomerID = async (req, res) => {
  ...
  try {
    ...
    if (!cart) {
      return res
        .status(404)
        .json({ message: "Cart not found for the given customer ID." });
    }

    const cartDetails = await cartMethods.getAllCarts();

    const customerCart = cartDetails.find(
    ...
    if (!customerCart) {
      return res
        .status(404)
        .json({ message: "Cart details not found for the given customer ID." });
    }
